...
{
    public enum DaxScriptType { Query, Expression }

    public enum LocalDefinitionType
    {
        Variable, Measure
    }
    public class DaxParseTreeNavigator : ParseTreeNavigator
    {
        public DaxParseTreeNavigator(ParseTree tree)
            :base(tree)
        {
        }
        ...
        {
            return DFTraverse(_tree).FirstOrDefault(x => x != null && x.Term.Name == DaxGrammar.NONTERM_EXPRESSION);
        }
