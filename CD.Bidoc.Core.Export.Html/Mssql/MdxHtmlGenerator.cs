...
{
    public class MdxHtmlGenerator : IGraphNodeHtmlGenerator
    {
        private LinkModeEnum _linkMode;

        public MdxHtmlGenerator(LinkModeEnum linkMode)
        {
            _linkMode = linkMode;
        }
        ...
        private List<Tuple<MdxFragmentElement, int>> GetAllDefinitionSegments(MdxFragmentElement node, int level = 0)
        {
            return node.Children.Select(x => new Tuple<MdxFragmentElement, int>((MdxFragmentElement)x, level)).Union(node.Children.SelectMany(ds => GetAllDefinitionSegments((MdxFragmentElement)ds, level + 1))).ToList();
        }
        ...
        public string GenerateHtmlDocument(IDependencyGraph graph, IDependencyGraphNode node)
        {
            ...
            var segments = GetAllDefinitionSegments((MdxFragmentElement)node.ModelElement);
            ...
            if (node.ModelElement.RefPath.Path == "SSASServer[@Name='RJ-THINK']/Db[@Name='Manpower_SSAS']/Cube[@Name='Manpower']/MdxScript[@Name='MdxScript']/CalculatedMeasure[@Name='Nonpersonnel Costs']")
            {
            ...
            foreach (var l in segments)
            {
                ...
                if (l.Item1.Reference is MdxFragmentElement)
                {
                    if (!(targetFileDictionary.ContainsKey(sourcePath) && targetFileDictionary[sourcePath] == (/*(MdxFragmentElement)*/(l.Item1.Reference)).RefPath.GetHtmlRefPath())) // restrict to scriptroot?
                    {
                        targetFileDictionary.Add(sourcePath, (/*(MdxFragmentElement)*/(l.Item1.Reference)).RefPath.GetHtmlRefPath()); // script root?
                    }
                }
            }
            ...
            foreach (var segment in segments)
            {
                ...
                if (!targetDictionary.ContainsKey(sourcePath))
                {
                    ...
                    {
                        replacement = string.Format("<a href=\"{0}\"", targetPath);
                        endReplacement = "</a>";
                    }
        ...
        private string InsertCodeSegmentPositionsIntoDefinition(MssqlModelElement node, List<Tuple<MdxFragmentElement, int>> segments, out List<InsertedTag> insertionOrder)
        {
            ...
            var startPositions = segments.Select(x => new InsertedTag() { Pos = x.Item1.OffsetFrom, Level = x.Item2, Tag = "START_" + x.Item1.RefPath.GetHtmlRefPath() + "|", HtmlId = x.Item1.RefPath.GetHtmlRefPath() }).ToList();
            var endPositions = segments.Select(x => new InsertedTag() { Pos = x.Item1.OffsetFrom + x.Item1.Length, Level = x.Item2, Tag = "END_" + x.Item1.RefPath.GetHtmlRefPath() + "|", HtmlId = x.Item1.RefPath.GetHtmlRefPath() }).ToList();
